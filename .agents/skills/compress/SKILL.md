...
1. Infer the target problem from the current conversation. If it is ambiguous, ask which problem to compress.
2. Read the problem's solution file under `src/main/java/com/albin/neetcode/`. Skim its test file if edge cases matter.
3. Draft exactly five bullets:
...
4. Before inserting, search `NOTES.md` for an existing `### ProblemName` entry. If one exists, do not duplicate or overwrite it; report that it is already compressed unless the developer explicitly asks to revise it.
5. In `NOTES.md`, find the `##` heading matching the solution Javadoc's `Category: …` line and insert the `### ProblemName` entry, ordered within the section like the README table.
6. Create missing headings in NeetCode roadmap order; keep all 18 headings even when empty — empty means "unsolved gap."
